...
#if !defined(EXCLUDE_DSP_BILINEAR_INTERP_Q31)

#include "dsplib_primitives.h"

#pragma Code("dsplib_dsp_bilinear_interp_q31")
/**
 * q31 BILINEAR INTERPOLATION PROCESSING FUNCTION
 *
...
q31_t dsp_bilinear_interp_q31(const matrix_q31_t * pGrid, int32_t x, int32_t y)
  {
    q31_t acc1;
    q31_t xfract, yfract;
    q31_t x1, x2, y1, y2;
    uint32_t x_index, y_index;
    q31_t *pTableData = pGrid->pData;
    ...
    //res = x1*(1-xfract)*(1-yfract) + x2*xfract*(1-yfract) + y1*(1-xfract)*yfract + y2*xfract*yfract;
    {
...
#if 0 // fix STAR: 9000676924 accuracy 0x7fffffff is not exact ONE 0x80000000
        q31_t minus_yfract = dspfx_add_q31(one - yfract, 1);
        q31_t minus_xfract = dspfx_add_q31(one - xfract, 1);

        q31_t out  =  (xfract == 0) ? x1 : dspfx_mpy_q31(x1, minus_xfract);
        q31_t out2 =  (yfract == 0) ? x2 : dspfx_mpy_q31(x2, minus_yfract);
        q31_t out3 =  (xfract == 0) ? y1 : dspfx_mpy_q31(y1, minus_xfract);
        q31_t out4 =  dspfx_mpy_q31(y2, xfract);

        dspfx_a64mul_q31(out2,xfract,acc);
        dspfx_a64mac_q31(out, minus_yfract, acc);
        if (yfract == 0) dspfx_a64mac_q31(out, 1, acc);
        dspfx_a64mac_q31(out3,yfract,acc);
        acc= dspfx_a64mac_q31(out4,yfract,acc);
#else
        ...
        dspfx_a64mac_q31(out3,yfract,acc);
        acc1= dspfx_a64mac_q31(out4,yfract,acc);
#endif
    }
...
#endif //!defined(EXCLUDE_DSP_BILINEAR_INTERP_Q31)
