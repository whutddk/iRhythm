...
#if !defined(EXCLUDE_DSP_MAT_SUB_Q15)
...
#pragma Code("dsplib_dsp_mat_sub_q15")


static _Inline dsp_status _DSPLIB_XYCC risc_dsp_mat_sub_q15(const matrix_q15_t *pSrcA,const matrix_q15_t *pSrcB, matrix_q15_t *pDst)
{
    q15_t    *A, *B, *C;
    ...
    C = pDst->pData;
    /* C = A - B */
    dsp_sub_q15((PTR(q15_t))A,(PTR(q15_t))B,(PTR(q15_t))C, pDst->size);
...
dsp_status _DSPLIB_XYCC dsp_mat_sub_q15(const matrix_q15_t *pSrcA, const matrix_q15_t *pSrcB, matrix_q15_t *pDst)
{
    ...
    if (nSamples < XY_MAT_SUB_THRESHOLD || nSamples > XY_BLOCK_SIZE) {
#endif
        return risc_dsp_mat_sub_q15(pSrcA, pSrcB, pDst);
#if defined(_ARC_AS210_nomac) 
    } else {
        ...
        dsp_mat_sub_q15_XXtoY((const matrix_q15_t *)&xySrcA, (const matrix_q15_t *)&xySrcB, &xyDst);
...
#endif //!defined(EXCLUDE_DSP_MAT_SUB_Q15)
