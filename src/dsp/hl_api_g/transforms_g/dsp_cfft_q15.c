...
#if !defined(EXCLUDE_DSP_CFFT_Q15)
#pragma Code("dsplib_dsp_cfft_q15")
 ...
 * \param pState - pointer to state vector buffer. length of state buffer should be obtained through "dsp_cfft_getsize_q15()" function.
 * \result dsp_status
 */ 
...
dsp_status _DSPLIB_XYCC dsp_cfft_init_q15(cfft_q15_t* handle, uint32_t length, PTR(uint32_t) pState)
{
    ...
    if (dsp_cfft_q15_twd_tab[order - DSP_MIN_TRANSFORM_ORDER] == 0)
    ...
    memcpy(handle->ptwd, dsp_cfft_q15_twd_tab[order - DSP_MIN_TRANSFORM_ORDER], dsp_cfft_getsize_q15(length));
#else
    handle->ptwd = (q15_t*)dsp_cfft_q15_twd_tab[order - DSP_MIN_TRANSFORM_ORDER];
#endif    
    handle->tab_bit_rev = (int16_t*) dsp_cfft_bitrev_tab[order - DSP_MIN_TRANSFORM_ORDER];
    handle->temp_buffer = (q15_t*)0;
    handle->shiftBits = 0;
...
static _Inline void risc_dsp_cfft_radix2_q15(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
...
#define RADIX8_BASEOP(w00, w01, w02, w03) \
{                                         \
    uint32_t w0 = COMPLEX_CONST_UI32(0x0000, 0x7fff); \
    uint32_t w1 = COMPLEX_CONST_UI32(0xa57e, 0x5a82); \
    uint32_t w2 = COMPLEX_CONST_UI32(0x8000, 0x0000); \
    uint32_t w3 = COMPLEX_CONST_UI32(0xa57e, 0xa57e); \
                  \
	/* 1st pass */\
	_cbflyhf1r(a0); \
...
#define RADIX4_BASEOP(w00, w01) \
{ \
    uint32_t w0 = COMPLEX_CONST_UI32(0x0000, 0x7fff); \
    uint32_t w1 = COMPLEX_CONST_UI32(0x8000, 0x0000); \
	_cbflyhf1r(a0); \
	/* 1st pass */            \
	a0 = _cbflyhf0r(w00, a1); \
	a1 = _cbflyhf1r(a2);      \
...
void dsp_cfft_radix2_q15_av2em(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
    int32_t nBlocks, nPoints;
    int32_t nLength;
    int32_t i, j, k, m;
...
void dsp_cfft_mradix_q15_av2em(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
    uint32_t* p_input         = (uint32_t*)&input[0];
    uint32_t* p_output        = (uint32_t*)&output[0];
    uint32_t* twfs            = (uint32_t*)&handle->ptwd[0];
    int32_t i, n_blocks, n_dfts;
    ...
    while (n_stages >= 3)
    {
        ...
        int32_t b_tw_stride;
        int32_t dft;
        for(dft = 0, b_tw_stride = 0; dft < n_dfts; dft++, b_tw_stride+=(n_blocks*sizeof(uint32_t)))
        {
            ...
            uint32_t* pin  = &p_output[dft];
            uint32_t* poutput = &p_output[dft];
            int32_t b_in_stride = n_dfts*4; // output stride in bytes
            for(block = 0; block < n_blocks; block++)
            {
    ...
    if (n_stages >= 2)
    {
        ...
        uint32_t ndft1 = n_dfts*1;
        uint32_t ndft2 = n_dfts*2;
		uint32_t ndft3 = n_dfts*3;
    ...
    if (n_stages >= 1)
    {
        ...
        uint32_t* __restrict b = &p_output[n_dfts];
        uint32_t* __restrict w = &twfs[0];
        _cbflyhf1r(*a);
        for (i = 0; i < n_dfts >> 1;  i++) // unroll by 2
        {
            uint32_t w1, w2;
            w1 = w[0];
            w2 = w[1];
...
#endif // defined (USE_RADIX2_Q15_FFT)
#endif //__Xdsp_complex
#else  // ARCv2EM XY version of FFT

// if defined _ARCv2EM_XY_AGU_MEDIUM && 0
//_Inline static 
...
//_Inline static 
uint32_t _agu_bitrev(uint32_t addr, uint32_t inc) 
{
    return revbin(revbin(addr) + revbin(inc));
};
...
void dsp_cfft_radix2_q15_av2em(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
    ...
    _DSPLIB_store_regs(3, 8, 4);

    _set_agu_ap0((uint32_t)input);
    _set_agu_ap1((uint32_t)poutput);
    ...
    n_blocks = N >> 1;
    if (input == poutput){ // inplace processing
...
void dsp_cfft_radix2_q15_av2em_nobfly(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
...
void dsp_cfft_4_q15_av2em(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
...
void dsp_cfft_8_q15_av2em(const cfft_q15_t* handle, const q15_t* input, q15_t* output)
{
    uint32_t *pin = (uint32_t*) input;
    uint32_t *pout = (uint32_t*) output;
...
#pragma Off_inline(dsp_cfft_4_q15_av2em, dsp_cfft_8_q15_av2em)
...
#define XY_FFT_RADIX2_Q15_THRESHOLD (((XY_REGION_SIZE/4))/sizeof(q15_t))

/**
 * Complex FFT "process" function for q15 data type
 *
 * \param handle - pointer to transform instance structure
...
void _DSPLIB_XYCC dsp_cfft_q15(const cfft_q15_t* handle, const PTR(q15_t) input, PTR(q15_t) output)
{
    ...
    if (handle->length == 4){
        dsp_cfft_4_q15_av2em(handle, input, output);
    } else
#endif
#if defined(DSP_CFFT_Q15_8)
    if (handle->length == 8){
        dsp_cfft_8_q15_av2em(handle, input, output);
    } else
#endif
    {
#if defined(USE_RADIX2_Q15_FFT)
        dsp_cfft_radix2_q15_av2em(handle, input, output);          
#else
        dsp_cfft_mradix_q15_av2em(handle, input, output);
#endif
    }
#else
#if defined(_ARC_AS210_mac16)
    if (handle->length > XY_FFT_RADIX2_Q15_THRESHOLD) {
#endif
#if defined(__Xdsp) && defined(_ARCv2EM_XY_AGU_MEDIUM) // Use ARCv2EM XY version without FFT accelerator
    	dsp_cfft_radix2_q15_av2em_nobfly(handle, input, output);
#else
        risc_dsp_cfft_radix2_q15(handle, input, output);
#endif
#if defined(_ARC_AS210_mac16) 
    } else {
...
#endif //!defined(EXCLUDE_DSP_CFFT_Q15)

#if !defined(EXCLUDE_DSP_CFFT_RADIX2_Q15)
#pragma Code("dsplib_dsp_cfft_radix2_q15")
...
*
* \param handle - pointer to transform instance structure
* \param temp0 - reseved parameter
* \param length - transform size
* \param flag_ifft - flag that enables inversed tranform
...
dsp_status     dsp_cfft_radix2_init_q15 (cfft_q15_t* handle, uint16_t length, uint8_t invFlag, uint8_t bitReverse_flag)
{
    ...
    if (0 == invFlag)
    {
        status=dsp_cfft_init_q15(handle, length, (PTR(uint32_t)) pState);
        handle->shiftBits = order; // use 1/N multiplier for the FORWARD FFT
        handle->scaling = DSP_TRANSFORM_SCALING_SCALED;
    }
    else
    {
        status=dsp_cifft_init_q15(handle, length, (PTR(uint32_t)) pState);
...
void _DSPLIB_XYCC dsp_cfft_radix2_q15( const cfft_q15_t *handle,  PTR(q15_t) pInOut)
{
    if( 0 == handle->invFlag)
    {
        dsp_scale_q15(pInOut, 0x4000, 0, pInOut, handle->length*2); // Scale by 0.5
        dsp_cfft_q15(handle,  pInOut, pInOut);
    }
    else
    {
        dsp_scale_q15(pInOut, 0x4000, 0, pInOut, handle->length*2); // Scale by 0.5
        dsp_cifft_q15( handle,  pInOut, pInOut);
    }
}


#endif // !defined(EXCLUDE_DSP_CFFT_RADIX2_Q15)
