...
#if !defined(EXCLUDE_DSP_DCT3_Q31)
#pragma Code("dsplib_dsp_dct3_q31")
...
/**
 * DCT Type III instance initialization function for q31 data type
 *
 * \param handle - pointer to transform instance structure
 * \param length - transform size
 * \param pState - pointer to state vector buffer. length of state buffer should be obtained through "dsp_dct3_getsize_q31()" function.
 */
...
static void _DSPLIB_XYCC risc_dsp_dct3_q31(const dct3_q31_t* handle, const q31_t* input, q31_t* output)
{
    int32_t i;
	...
	_set_agu_ap0(input + 1);
	_set_agu_ap1(input + L - 1);
	_set_agu_ap2(output + 2);
    ...
    for (i = 1; i < N; i++) 
    {
        ...
        _st_agu_u4(dspfx_a64mac_q31(_agu_u2(), xSin, acc));
                   dspfx_a64mul_q31(_agu_u3(), xCos, acc);
        _st_agu_u4(dspfx_a64msu_q31(_agu_u1(), xSin, acc));
    }
    xCos = _agu_u5();

    COMPLEX_RE(output, 0) = dspfx_add_q31(input[0], dspfx_mpy_q31(input[N], xCos));
    COMPLEX_IM(output, 0) = dspfx_sub_q31(input[0], dspfx_mpy_q31(input[N], xCos));

	_set_agu_ap0(output + 2);
	_set_agu_ap1(output + L - 2);
    ...
    for (i = 1; i < N>>1; i++) 
	{
        ...
        _st_agu_u0(dspfx_sub_q31(a1, a5));
        _st_agu_u0(dspfx_add_q31(a6, a4));
        _st_agu_u2(dspfx_add_q31(a1, a5));
        _st_agu_u4(dspfx_sub_q31(a6, a4));
    }

    dsp_cfft_q31(&handle->hfft, (PTR(q31_t))output, (PTR(q31_t))buffer);

	_set_agu_ap0(buffer);
	_set_agu_ap1(buffer + L - 1);
	_set_agu_ap2(output);

	_set_agu_mod0(0, AGU_DT_q31, AGU_AD_INC_q31(1));
	_set_agu_mod1(1, AGU_DT_q31, AGU_AD_DEC_q31(1));
	_set_agu_mod2(2, AGU_DT_q31, AGU_AD_INC_q31(2));
...
#pragma Off_inline(risc_dsp_dct3_q31)

#else

static _Inline void risc_dsp_dct3_q31(const dct3_q31_t* handle, const PTR(q31_t) input, PTR(q31_t) output) 
{
    int32_t i;
    ...
    for (i = 1; i < N; i++) 
    {
        xCos = *pTWD4++;
        xSin = *pTWD4++;
                                dspfx_a64mul_q31(input[i],     xCos, acc);
        COMPLEX_RE(buffer, i) = dspfx_a64mac_q31(input[L - i], xSin, acc);
                                dspfx_a64mul_q31(input[L - i], xCos, acc);
        COMPLEX_IM(buffer, i) = dspfx_a64msu_q31(input[i],     xSin, acc);
    }
    xCos = *pTWD4;

    COMPLEX_RE(buffer, 0) = dspfx_add_q31(input[0], dspfx_mpy_q31(input[N], xCos));
    COMPLEX_IM(buffer, 0) = dspfx_sub_q31(input[0], dspfx_mpy_q31(input[N], xCos));
    ...
    for (i = 1; i < N>>1; i++) {
        ...
        a5 = dspfx_a64mac_q31(a3, xSin, acc);
             dspfx_a64mul_q31(a2, xSin, acc);
        a6 = dspfx_a64msu_q31(a3, xCos, acc);
        COMPLEX_RE(buffer, i) = dspfx_sub_q31(a1, a5);
        COMPLEX_IM(buffer, i) = dspfx_add_q31(a6, a4);
        COMPLEX_RE(buffer, N - i) = dspfx_add_q31(a1, a5);
        COMPLEX_IM(buffer, N - i) = dspfx_sub_q31(a6, a4);
    }
...
#define XY_DCT3_Q31_THRESHOLD (((XY_REGION_SIZE/4))/sizeof(q31_t))

/**
 * DCT Type III "process" function for q31 data type
 *
 * \param handle - pointer to transform instance structure
...
{
#if defined(_ARC_AS210_mac32)
    if (handle->length > XY_DCT3_Q31_THRESHOLD) {
#endif
        risc_dsp_dct3_q31(handle, input, output);
#if defined(_ARC_AS210_mac32)
    } else {
        cfft_srad_q31_xy_t pInst;

        memcpy_StoY_int32(XY_MAP_INPUT, (const int32_t * )input, handle->length);

        dsp_dct3_preprocess_init_q31_cfY(
            (dct3_q31_xy_t*)&handle->dct3_xy,
            handle->length,
            0);
        dsp_dct3_preprocess_q31_YtoX_cfY_MAC32x32(
            &handle->dct3_xy,
            XY_MAP_INPUT,
            XY_MAP_TMP);
        ...
        dsp_dct3_postprocess_q31_YtoX(
            XY_MAP_TMP, 
            XY_MAP_OUT, 
            handle->length);

        memcpy_XtoS_int32((int32_t*)output, XY_MAP_OUT , handle->length);
    }
#endif
}
...
#endif //!defined(EXCLUDE_DSP_DCT3_Q31)
