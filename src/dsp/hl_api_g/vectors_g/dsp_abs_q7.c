...
#if !defined(EXCLUDE_DSP_ABS_Q7)

#include "dsplib_primitives.h"

#pragma Code("dsplib_dsp_abs_q7")

#if defined _ARCv2EM_XY_AGU_SMALL && defined __FXAPI__

void _DSPLIB_XYCC dsp_abs_q7(const __xy q7_t * __restrict pSrc, __xy q7_t * __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) {
        _st_agu_u1(_vpack2hblf(fx_q31_cast_v2q15(fx_abs_v2q15(fx_v2q15_cast_q31(_agu_u0())))));
    }
...
/**
 * Q7 vector absolute value. Computes the 
...
void dsp_abs_q7 (const q7_t* __restrict  pSrc, q7_t* __restrict  pDst, uint32_t nSamples)
{
    int32_t sample;

#if (!(defined(ABS_VEC4_Q7)))
        ...
        || ((uint32_t)nSamples & 0x3) )
    {
        for (sample = 0; sample < nSamples; sample++)
            pDst[sample] = dspfx_abs_q7(pSrc[sample]);
    }
    else //use vector instruction
    {
        for (sample = 0; sample < (int32_t)(nSamples/4); sample++) {
            ...
            ta02 = dspfx_abs_v2q15(ta02);
            ta13 = dspfx_abs_v2q15(ta13);
...
#endif //!defined(EXCLUDE_DSP_ABS_Q7)
