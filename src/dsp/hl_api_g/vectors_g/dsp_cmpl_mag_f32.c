...
#if !defined(EXCLUDE_DSP_CMPL_MAG_F32)

#include "dsplib.h"
#include "dsplib_primitives.h"

#pragma Code("dsplib_dsp_cmpl_mag_f32")
/**
 * Computes the F32 magnitude of complex numbers stored in a vector                      
...
void _DSPLIB_XYCC dsp_cmpl_mag_f32(const __xy f32_t* __restrict pSrc,  __xy f32_t* __restrict pDst,  uint32_t nSamples)
{
    uint32_t i;
    f32_t acc;
    ...
    for (i=0; i < nSamples; i++)
    {
        ...
        acc += (_as_float(_agu_u1())) * (_as_float(_agu_u2()));
        _st_agu_u0(_as_uint32(dsp_sqrt_f32(acc)));
    }
...
void dsp_cmpl_mag_f32(const PTR(f32_t) __restrict pSrc,  PTR(f32_t) __restrict pDst,  uint32_t nSamples)
{
    uint32_t i;
    f32_t acc;
    for (i=0; i < nSamples; i++)
...
void dsp_cmpl_mag_f32(const f32_t* __restrict pSrc,  f32_t* __restrict pDst,  uint32_t nSamples)
{
    uint32_t i;
    f32_t acc;
    for (i=0; i < nSamples; i++)
...
#endif //!defined(EXCLUDE_DSP_CMPL_MAG_F32)
