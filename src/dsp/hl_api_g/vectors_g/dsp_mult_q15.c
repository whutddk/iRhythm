...
#if !defined(EXCLUDE_DSP_MULT_Q15)
...
#pragma Code("dsplib_dsp_mult_q15")

#if defined _ARCv2EM_XY && defined __FXAPI__

#if defined _XY_HAND_OPT && defined _ARCv2EM_XY_AGU_MEDIUM

void _DSPLIB_XYCC dsp_mult_q15(const __xy q15_t* __restrict pSrcA, const __xy q15_t* __restrict pSrcB, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) {
        _st_agu_u2(fx_q31_cast_v2q15(fx_v2q15_cast_v2a40(fx_v2a40_mpy_v2q15(fx_v2q15_cast_q31(_agu_u0()), fx_v2q15_cast_q31(_agu_u1())))));
    }
    if (nSamples & 1) {
...
void _DSPLIB_XYCC dsp_mult_q15(const __xy q15_t* __restrict pSrcA, const __xy q15_t* __restrict pSrcB, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    if (nSamples & 1) {
    	*pDst++ = fx_q15_cast_a40(fx_a40_mpy_q15(*pSrcA++,*pSrcB++));
    }
    ...
    for (i = 0; i < (nSamples >> 1); i++) {
        *pvDst++ = fx_v2q15_cast_v2a40(fx_v2a40_mpy_v2q15(*pvSrcA++, *pvSrcB++));
    }
}
...
static _Inline void risc_dsp_mult_q15(const q15_t* __restrict pSrcA, const q15_t* __restrict pSrcB, q15_t* __restrict pDst, uint32_t nSamples)
{
    int32_t sample;
#if !defined(dspfx_mpy_v2q15)
    // branch that calculates abs with plain loop
    INIT_MACMODE();
    for (sample = 0; sample < (int32_t)nSamples; sample++)
    {
        *pDst++ = dspfx_mpy_q15(*pSrcA++, *pSrcB++);
    }
#else
    // branch that calculates multiplication with vector instruction
...
#endif
    {
        ...
        if (startSample) pDst[0] = dspfx_mpy_q15(pSrcA[0],pSrcB[0]);
        ...
        for (sample = 0; sample < N; sample++) {
            uint32_t acc;
            acc = dspfx_mpy_v2q15(*pSrcA_32++, *pSrcB_32++);
            *pDst_32++ = (acc);
        }
        
        // process last sample if it is needed
        sample = (N << 1) + startSample;
        if (sample < (int32_t)nSamples) //'tail'
            pDst[nSamples-1] = dspfx_mpy_q15(pSrcA[nSamples-1],pSrcB[nSamples-1]);
...
/**
 * Q15 element-by-element multiplication of two vectors.
 *
...
void dsp_mult_q15(const q15_t* __restrict pSrcA, const q15_t* __restrict pSrcB, q15_t* __restrict pDst, uint32_t nSamples)
{
#if defined(_ARC_AS210_mac16)
    if (nSamples < XY_MULT_THRESHOLD) {
#endif
        // call to primitives based version of mult
        risc_dsp_mult_q15(pSrcA, pSrcB, pDst, nSamples);
#if defined(_ARC_AS210_mac16)
    } else {
        ...
        while (lastSize) 
        {
            ...
            // call to XY based version of mult
            dsp_mult_q15_XXtoY_MAC16x16(pxy_input0, pxy_input1, XYMAP_OUTPUT, currentSize);
...
#endif //!defined(EXCLUDE_DSP_MULT_Q15)
