...
#if !defined(EXCLUDE_DSP_NEGATE_Q15)
...
#pragma Code("dsplib_dsp_negate_q15")

#if defined _ARCv2EM_XY && defined __FXAPI__

#if defined _XY_HAND_OPT && defined _ARCv2EM_XY_AGU_SMALL

void _DSPLIB_XYCC dsp_negate_q15(const __xy q15_t * __restrict pSrc, __xy q15_t * __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) {
        _st_agu_u1(fx_q31_cast_v2q15(fx_neg_v2q15(fx_v2q15_cast_q31(_agu_u0()))));
    }
    if (nSamples & 1) {
...
static _Inline void risc_dsp_negate_q15(const q15_t* __restrict pSrc,  q15_t* __restrict pDst, uint32_t nSamples)
{
    int32_t sample;
#if !(defined(dspfx_neg_v2q15))
...
#else
    // branch that calculates vector negate with vector instruction
...
#endif
    {
        ...
        if (startSample) *pDst = dspfx_neg_q15(*pSrc);
...
/**
 * Q15 negate of the elements of a vector.
 *
...
void dsp_negate_q15(const q15_t* __restrict pSrc,  q15_t* __restrict pDst, uint32_t nSamples)
{
#if defined(_ARC_AS210_nomac)
    if (nSamples < XY_NEGATE_THRESHOLD) {
#endif
        // call to primitives based version of vector negate
        risc_dsp_negate_q15(pSrc, pDst, nSamples);
#if defined(_ARC_AS210_nomac)
    } else {
        ...
        while (lastSize) 
        {
            ...
            dsp_negate_q15_XtoY(pxy_input, XYMAP_OUTPUT, currentSize);
...
#endif //!defined(EXCLUDE_DSP_NEGATE_Q15)
