...
#if !defined(EXCLUDE_DSP_SHIFT_Q15)
...
#pragma Code("dsplib_dsp_shift_q15")

#if defined _ARCv2EM_XY && defined __FXAPI__

#if defined _XY_HAND_OPT && defined _ARCv2EM_XY_AGU_SMALL

void _DSPLIB_XYCC dsp_shift_q15(const __xy q15_t* __restrict pSrc, int8_t  shiftBits, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) {
        _st_agu_u1(fx_q31_cast_v2q15(fx_asl_v2q15(fx_v2q15_cast_q31(_agu_u0()), v2i16_shiftBits)));
    }
    if (nSamples & 1) {
...
void _DSPLIB_XYCC dsp_shift_q15(const __xy q15_t* __restrict pSrc, int8_t  shiftBits, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    if (nSamples & 1) {
    	*pDst++ = fx_asl_q15(*pSrc++, shiftBits);
    }
    pvSrc = (__xy v2q15_t*) pSrc;
    pvDst = (__xy v2q15_t*) pDst;
    for (i = 0; i < (nSamples >> 1); i++) {
        *pvDst++ = fx_asl_v2q15(*pvSrc++, v2i16_shiftBits);
    }
}
...
// primitives based version of vector shift
static _Inline void risc_dsp_shift_q15(const q15_t* __restrict pSrc, int8_t  shiftBits, q15_t* __restrict pDst, uint32_t nSamples)
{
    int32_t sample;
#if !defined(dspfx_asl_v2q15_n)
    // branch that calculates shift with plain loop
    for (sample = 0; sample < (int32_t)nSamples; sample++)
    {
        *pDst++ = dspfx_asl_q15(*pSrc++, shiftBits);
    }
#else
...
#endif
    {
        ...
        if (startSample) pDst[0] = dspfx_asl_q15(pSrc[0], shiftBits);
...
/**
 * Shifts the elements of a Q15 vector by a specified
...
void dsp_shift_q15(const q15_t* __restrict pSrc, int8_t  shiftBits, q15_t* __restrict pDst, uint32_t nSamples)
{
#if defined(_ARC_AS210_nomac)
    if (nSamples < XY_SHIFT_THRESHOLD) {
#endif
        // call to primitives based version of vector shift
        risc_dsp_shift_q15(pSrc, shiftBits, pDst, nSamples);
#if defined(_ARC_AS210_nomac)
    } else {
        ...
        while (lastSize) 
        {
            ...
            // call to XY based version of vector shift
            dsp_shift_q15_XtoY(pxy_input, shiftBits, XYMAP_OUTPUT, currentSize);
...
#endif //!defined(EXCLUDE_DSP_SHIFT_Q15)
