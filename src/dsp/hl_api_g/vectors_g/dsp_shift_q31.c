...
#if !defined(EXCLUDE_DSP_SHIFT_Q31)
...
#pragma Code("dsplib_dsp_shift_q31")
// primitives based version of vector shift
static _Inline void _DSPLIB_XYCC risc_dsp_shift_q31(const PTR(q31_t) __restrict pSrc, int8_t  shiftBits, PTR(q31_t) __restrict pDst, uint32_t nSamples)
{
    int32_t sample;
    for (sample = 0; sample < (int32_t)nSamples; sample++){
        *pDst++ = dspfx_asl_q31(*pSrc++, shiftBits);
    }
}
...
void _DSPLIB_XYCC dsp_shift_q31(const PTR(q31_t) __restrict pSrc, int8_t shiftBits, PTR(q31_t) __restrict pDst, uint32_t nSamples)
{
    ...
    if (nSamples < XY_SHIFT_THRESHOLD) {
        ...
        risc_dsp_shift_q31(pSrc, shiftBits, pDst, nSamples);
#if defined(_ARC_AS210_nomac)
    } else {
        int32_t currentSize;
        ...
        while (lastSize) 
        {
            ...
            dsp_shift_q31_XtoY(XYMAP_INPUT, shiftBits, XYMAP_OUTPUT, currentSize);
...
#endif //!defined(EXCLUDE_DSP_SHIFT_Q31)
