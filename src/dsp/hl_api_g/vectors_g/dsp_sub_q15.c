...
#if !defined(EXCLUDE_DSP_SUB_Q15)
...
#pragma Code("dsplib_dsp_sub_q15")

#if defined _ARCv2EM_XY && defined __FXAPI__

#if defined _XY_HAND_OPT && defined _ARCv2EM_XY_AGU_SMALL 

void _DSPLIB_XYCC dsp_sub_q15(const __xy q15_t* __restrict pSrcA, const __xy q15_t* __restrict pSrcB, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) 
    {
        _st_agu_u2(fx_q31_cast_v2q15(fx_sub_v2q15(fx_v2q15_cast_q31(_agu_u0()), fx_v2q15_cast_q31(_agu_u1()))));
    }
    if (nSamples & 1) {
...
void _DSPLIB_XYCC dsp_sub_q15(const __xy q15_t* __restrict pSrcA, const __xy q15_t* __restrict pSrcB, __xy q15_t* __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < (nSamples >> 1); i++) {
        *pvDst++ = fx_sub_v2q15(*pvSrcA++, *pvSrcB++);
    }
}
...
static _Inline void risc_dsp_sub_q15(const q15_t* __restrict pSrcA, const q15_t* pSrcB, q15_t* __restrict pDst, uint32_t nSamples)
{
    int32_t sample;
#if !defined(dspfx_sub_v2q15)
    // branch that calculates abs with plain loop
    for (sample = 0; sample < (int32_t)nSamples; sample++){
        *pDst++ = dspfx_sub_q15(*pSrcA++, *pSrcB++);
    }
#else
    // branch that calculates subtraction with vector instruction
...
#endif
    {
        ...
        if (startSample) *pDst = dspfx_sub_q15(*pSrcA,*pSrcB);
        ...
        for (sample = 0; sample < N; sample++)
            *pDst_32++ = dspfx_sub_v2q15(*pSrcA_32++, *pSrcB_32++);

        // process last sample if it is needed
        sample = (N << 1) + startSample;
        if (sample < (int32_t)nSamples)
            pDst[nSamples-1] = dspfx_sub_q15(pSrcA[nSamples-1],pSrcB[nSamples-1]);
    }
#endif
}
...
/**
 * Q15 element-by-element subtraction of two vectors.
 *
...
void dsp_sub_q15(const q15_t* __restrict pSrcA, const q15_t* __restrict pSrcB, q15_t* __restrict pDst, uint32_t nSamples)
{
#if defined(_ARC_AS210_nomac) 
    if (nSamples < XY_SUB_THRESHOLD) {
#endif
        // call to primitives based version of sub
        risc_dsp_sub_q15(pSrcA, pSrcB, pDst, nSamples);
#if defined(_ARC_AS210_nomac) 
    } else {
        ...
        while (lastSize) 
        {
            ...
            dsp_sub_q15_XXtoY(pxy_input0, pxy_input1, XYMAP_OUTPUT, currentSize);
...
#endif //!defined(EXCLUDE_DSP_SUB_Q15)
