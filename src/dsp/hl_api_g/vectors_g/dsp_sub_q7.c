...
#if !defined(EXCLUDE_DSP_SUB_Q7)

#include "dsplib_primitives.h"

#pragma Code("dsplib_dsp_sub_q7")

#if defined _ARCv2EM_XY_AGU_SMALL && defined __FXAPI__

void _DSPLIB_XYCC dsp_sub_q7(const __xy q7_t* __restrict pSrcA, const __xy q7_t* __restrict pSrcB, __xy q7_t* __restrict pDst, uint32_t nSamples)
{
    ...
    for (i = 0; i < N; i++) {
        _st_agu_u2(_vpack2hblf(fx_q31_cast_v2q15(fx_sub_v2q15(fx_v2q15_cast_q31(_agu_u0()), fx_v2q15_cast_q31(_agu_u1())))));
    }
    if (nSamples & 1) {
...
/**
 * Q7 element-by-element subtraction of two vectors.
 *
...
void dsp_sub_q7 (const q7_t* __restrict pSrcA, const q7_t* __restrict pSrcB, q7_t* __restrict pDst, uint32_t nSamples)
{
    int32_t sample;

#if !(defined(SUB_VEC4_Q7))
        ...
        || ((uint32_t)nSamples & 0x3) )
    {
        for (sample = 0; sample < nSamples; sample++)
            pDst[sample] = dspfx_sub_q7(pSrcA[sample], pSrcB[sample]);
    }
    else //use vector instruction
    {
        for (sample = 0; sample < (int32_t)(nSamples/4); sample++) {
            ...
            ta02 = dspfx_sub_v2q15(ta02, tb02);
            ta13 = dspfx_sub_v2q15(ta13, tb13);
    ...
    //inputs and ouput are not 'equally' (un)aligned -> use 1d cycle
    ...
    else //use vector instruction
    {
        ...
        for (sample = 0; sample < N; sample++) 
            *pDst_32++ = SUB_VEC4_Q7(*pSrcA_32++, *pSrcB_32++);
...
#endif //!defined(EXCLUDE_DSP_SUB_Q7)
