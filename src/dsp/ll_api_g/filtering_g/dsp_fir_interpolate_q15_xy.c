...
// Produce instance with: MAC16x16; pCoeffs - X; pIn - X; pOut - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_XtoX_cfXstY_MAC16x16")
...
void dsp_fir_interpolate_q15_XtoX_cfXstY_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    N = nSamples;
    factor = inst->factor;
    dly_line_size = inst->dly_line_size;
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ax2(cfs_ptr);
    _set_mx20((AM_16BIT|AM_OFFS(factor)));
    _set_mx21((AM_16BIT|AM_OFFS(factor + 1 - T)));
    ...
    if (dly_line_size < 2){
        ...
        _set_my00((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my01((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_y0_u0(), _x2_u0());
            // Move convolution result to output buffer
            _set_x1_u0( _macrdw(_y0_u0(), _x2_u1()));
        }
        // update pointers
        _set_ax2(cfs_ptr);
        _y0_u1();
...
// Produce instance with: MAC16x16; pCoeffs - X; pIn - Y; pOut - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_YtoX_cfXstY_MAC16x16")
...
void dsp_fir_interpolate_q15_YtoX_cfXstY_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ax1(cfs_ptr);
    _set_mx10((AM_16BIT|AM_OFFS(factor)));
    _set_mx11((AM_16BIT|AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_my10((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my11((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_y1_u0(), _x1_u0());
            // Move convolution result to output buffer
            _set_x0_u0( _macrdw(_y1_u0(), _x1_u1()));
        }
        // update pointers
        _set_ax1(cfs_ptr);
        _y1_u1();
...
// Produce instance with: MAC16x16; pIn - X; pOut - Y; pCoeffs - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_XtoY_cfXstY_MAC16x16")
...
void dsp_fir_interpolate_q15_XtoY_cfXstY_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_y1_u0(), _x1_u0());
            // Move convolution result to output buffer
            _set_y0_u0( _macrdw(_y1_u0(), _x1_u1()));
        }
        // update pointers
        _set_ax1(cfs_ptr);
        _y1_u1();
...
#pragma Code("dsplib_dsp_fir_interpolate_q15_YtoY_cfXstY_MAC16x16")
...
void dsp_fir_interpolate_q15_YtoY_cfXstY_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ax0(cfs_ptr);
    _set_mx00((AM_16BIT|AM_OFFS(factor)));
    _set_mx01((AM_16BIT|AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_my20((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my21((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_y2_u0(), _x0_u0());
            // Move convolution result to output buffer
            _set_y1_u0( _macrdw(_y2_u0(), _x0_u1()));
        }
        // update pointers
        _set_ax0(cfs_ptr);
        _y2_u1();
...
// Produce instance with: MAC16x16; pCoeffs - Y; pOut - X; pIn - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_XtoX_cfYstX_MAC16x16")
...
void dsp_fir_interpolate_q15_XtoX_cfYstX_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay0(cfs_ptr);
    _set_my00((AM_16BIT|AM_OFFS(factor)));
    _set_my01((AM_16BIT|AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx20((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx21((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_x2_u0(), _y0_u0());
            // Move convolution result to output buffer
            _set_x1_u0( _macrdw(_x2_u0(), _y0_u1()));
        }
        // update pointers
        _set_ay0(cfs_ptr);
        _x2_u1();
...
#pragma Code("dsplib_dsp_fir_interpolate_q15_YtoX_cfYstX_MAC16x16")
...
void dsp_fir_interpolate_q15_YtoX_cfYstX_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay1(cfs_ptr);
    _set_my10((AM_16BIT|AM_OFFS(factor)));
    _set_my11((AM_16BIT|AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx10((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx11((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_x1_u0(), _y1_u0());
            // Move convolution result to output buffer
            _set_x0_u0( _macrdw(_x1_u0(), _y1_u1()));
        }
        // update pointers
        _set_ay1(cfs_ptr);
        _x1_u1();
...
// Produce instance with: MAC16x16; pCoeffs - Y; pIn - X; pOut - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_XtoY_cfYstX_MAC16x16")
...
void dsp_fir_interpolate_q15_XtoY_cfYstX_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_x1_u0(), _y1_u0());
            // Move convolution result to output buffer
            _set_y0_u0( _macrdw(_x1_u0(), _y1_u1()));
        }
        // update pointers
        _set_ay1(cfs_ptr);
        _x1_u1();
...
// Produce instance with: MAC16x16; pCoeffs - Y; pIn - Y; pOut - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q15_YtoY_cfYstX_MAC16x16")
...
void dsp_fir_interpolate_q15_YtoY_cfYstX_MAC16x16(fir_intdec_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay2(cfs_ptr);
    _set_my20((AM_16BIT|AM_OFFS(factor)));
    _set_my21((AM_16BIT|AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx00((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx01((AM_16BIT|AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    iterations = dly_line_size - 1;
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            if (iterations & 1) _macrdw(_x0_u0(), _y2_u0());
            // Move convolution result to output buffer
            _set_y1_u0( _macrdw(_x0_u0(), _y2_u1()));
        }
        // update pointers
        _set_ay2(cfs_ptr);
        _x0_u1();
...
#pragma Code("dsplib_dsp_fir_interpolate_init_q15_cfXstY_MAC16x16")
...
dsp_status dsp_fir_interpolate_init_q15_cfXstY_MAC16x16(fir_intdec_q15_xy_t *inst, uint16_t nTaps, uint8_t intFactor,  uint32_t offs_coeffs, uint32_t offs_states)  
{
    ...
    inst->dly_line_size = nTaps/intFactor;
    _MEM_FILL_Y_Q15(offs_states, 0, nTaps);
    return DSP_ERR_OK;
}
// Produce instance with: MAC16x16; pCoeffs - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_init_q15_cfYstX_MAC16x16")
