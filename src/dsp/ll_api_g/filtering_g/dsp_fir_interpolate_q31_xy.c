...
// Produce instance with: MAC32x32; pCoeffs - X; pOut - X; pIn - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_XtoX_cfXstY_MAC32x32")
...
void dsp_fir_interpolate_q31_XtoX_cfXstY_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    _set_ax2(cfs_ptr);
    _set_mx20((AM_OFFS(factor)));
    _set_mx21((AM_OFFS(factor + 1 - T)));
    ...
    if (dly_line_size < 2){
        ...
        _set_my00((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my01((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_y0_nu(), _x2_nu());
            _set_x1_u0( _machflw(_y0_u0(), _x2_u1()));
        }
        // update pointers
        _set_ax2(cfs_ptr);
        _y0_u1();
...
#pragma Code("dsplib_dsp_fir_interpolate_q31_XtoY_cfXstY_MAC32x32")
...
void dsp_fir_interpolate_q31_XtoY_cfXstY_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ax1(cfs_ptr);
    _set_mx10((AM_OFFS(factor)));
    _set_mx11((AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_my10((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my11((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_y1_nu(), _x1_nu());
            _set_y0_u0( _machflw(_y1_u0(), _x1_u1()));
        }
        // update pointers
        _set_ax1(cfs_ptr);
        _y1_u1();
...
// Produce instance with: MAC32x32; pOut - X; pIn - X; pCoeffs - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_XtoX_cfYstX_MAC32x32")
...
void dsp_fir_interpolate_q31_XtoX_cfYstX_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay0(cfs_ptr);
    _set_my00((AM_OFFS(factor)));
    _set_my01((AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx20((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx21((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_x2_nu(), _y0_nu());
            _set_x1_u0( _machflw(_x2_u0(), _y0_u1()));
        }
        // update pointers
        _set_ay0(cfs_ptr);
        _x2_u1();
...
// Produce instance with: MAC32x32; pOut - Y; pIn - X; pCoeffs - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_XtoY_cfYstX_MAC32x32")
...
void dsp_fir_interpolate_q31_XtoY_cfYstX_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay1(cfs_ptr);
    _set_my10((AM_OFFS(factor)));
    _set_my11((AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx10((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx11((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_x1_nu(), _y1_nu());
            _set_y0_u0( _machflw(_x1_u0(), _y1_u1()));
        }
        // update pointers
        _set_ay1(cfs_ptr);
        _x1_u1();
...
// Produce instance with: MAC32x32; pOut - X; pIn - Y; pCoeffs - X; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_YtoX_cfXstY_MAC32x32")
...
void dsp_fir_interpolate_q31_YtoX_cfXstY_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_y1_nu(), _x1_nu());
            _set_x0_u0( _machflw(_y1_u0(), _x1_u1()));
        }
        // update pointers
        _set_ax1(cfs_ptr);
        _y1_u1();
...
// Produce instance with: MAC32x32; pCoeffs - X; pIn - Y; pOut - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_YtoY_cfXstY_MAC32x32")
...
void dsp_fir_interpolate_q31_YtoY_cfXstY_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ax0(cfs_ptr);
    _set_mx00((AM_OFFS(factor)));
    _set_mx01((AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_my20((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_my21((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_y2_nu(), _x0_nu());
            _set_y1_u0( _machflw(_y2_u0(), _x0_u1()));
        }
        // update pointers
        _set_ax0(cfs_ptr);
        _y2_u1();
...
// Produce instance with: MAC32x32; pIn - Y; pOut - X; pCoeffs - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_YtoX_cfYstX_MAC32x32")
...
void dsp_fir_interpolate_q31_YtoX_cfYstX_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_x1_nu(), _y1_nu());
            _set_x0_u0( _machflw(_x1_u0(), _y1_u1()));
        }
        // update pointers
        _set_ay1(cfs_ptr);
        _x1_u1();
...
// Produce instance with: MAC32x32; pOut - Y; pIn - Y; pCoeffs - Y; 

#pragma Code("dsplib_dsp_fir_interpolate_q31_YtoY_cfYstX_MAC32x32")
...
void dsp_fir_interpolate_q31_YtoY_cfYstX_MAC32x32(fir_intdec_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    cfs_ptr = inst->filter.xyptr_coeffs;
    _set_ay2(cfs_ptr);
    _set_my20((AM_OFFS(factor)));
    _set_my21((AM_OFFS(factor + 1 - T)));
    // Init XY pointer for filter delay line (modulo T addressing)
    ...
    if (dly_line_size < 2){
        ...
        _set_mx00((AM_MOD(dly_line_size)|AM_OFFS(1)));
        _set_mx01((AM_MOD(dly_line_size)|AM_OFFS(-1)));
    }
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        for (j = 0; j < factor; j++){
            ...
            // Move convolution result to output buffer
                        _macflw(_x0_nu(), _y2_nu());
            _set_y1_u0( _machflw(_x0_u0(), _y2_u1()));
        }
        // update pointers
        _set_ay2(cfs_ptr);
        _x0_u1();
...
#pragma Code("dsplib_dsp_fir_interpolate_init_q31_cfXstY_MAC32x32")
...
#pragma Code("dsplib_dsp_fir_interpolate_init_q31_cfYstX_MAC32x32")
