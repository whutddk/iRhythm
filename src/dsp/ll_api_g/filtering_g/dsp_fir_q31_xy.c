...
#pragma Code("dsplib_dsp_fir_q31_XtoX_cfXstY_MAC32x32")
...
void dsp_fir_q31_XtoX_cfXstY_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_y0_nu(), _x2_nu());
        _set_x1_u0( _machflw(_y0_nu(), _x2_u0()));
    }
...
#pragma Code("dsplib_dsp_fir_q31_YtoX_cfXstY_MAC32x32")
...
void dsp_fir_q31_YtoX_cfXstY_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_y1_nu(), _x1_nu());
        _set_x0_u0( _machflw(_y1_nu(), _x1_u0()));
    }
    // Save delay line offset
    inst->shift = _ay1() - inst->xyptr_states;
}
// Produce instance with: MAC32x32; pIn - X; pCoeffs - X; pOut - Y; 

#pragma Code("dsplib_dsp_fir_q31_XtoY_cfXstY_MAC32x32")
...
void dsp_fir_q31_XtoY_cfXstY_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_y1_nu(), _x1_nu());
        _set_y0_u0( _machflw(_y1_nu(), _x1_u0()));
    }
    // Save delay line offset
    inst->shift = _ay1() - inst->xyptr_states;
}
// Produce instance with: MAC32x32; pIn - Y; pCoeffs - X; pOut - Y; 

#pragma Code("dsplib_dsp_fir_q31_YtoY_cfXstY_MAC32x32")
...
void dsp_fir_q31_YtoY_cfXstY_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_y2_nu(), _x0_nu());
        _set_y1_u0( _machflw(_y2_nu(), _x0_u0()));
    }
    // Save delay line offset
...
#pragma Code("dsplib_dsp_fir_q31_XtoX_cfYstX_MAC32x32")
...
void dsp_fir_q31_XtoX_cfYstX_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_x2_nu(), _y0_nu());
        _set_x1_u0( _machflw(_x2_nu(), _y0_u0()));
    }
    // Save delay line offset
...
#pragma Code("dsplib_dsp_fir_q31_XtoY_cfYstX_MAC32x32")
...
void dsp_fir_q31_XtoY_cfYstX_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_x1_nu(), _y1_nu());
        _set_y0_u0( _machflw(_x1_nu(), _y1_u0()));
    }
    // Save delay line offset
    inst->shift = _ax1() - inst->xyptr_states;
}
// Produce instance with: MAC32x32; pIn - Y; pCoeffs - Y; pOut - X; 

#pragma Code("dsplib_dsp_fir_q31_YtoX_cfYstX_MAC32x32")
...
void dsp_fir_q31_YtoX_cfYstX_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_x1_nu(), _y1_nu());
        _set_x0_u0( _machflw(_x1_nu(), _y1_u0()));
    }
    // Save delay line offset
    inst->shift = _ax1() - inst->xyptr_states;
}
...
#pragma Code("dsplib_dsp_fir_q31_YtoY_cfYstX_MAC32x32")
...
void dsp_fir_q31_YtoY_cfYstX_MAC32x32(fir_q31_xy_t *inst, uint32_t offs_input, uint32_t offs_output, uint32_t nSamples)  
{
    ...
    for ( n = 0; n < N; n++ )
    {
        ...
        // Move convolution result to output buffer
                    _macflw(_x0_nu(), _y2_nu());
        _set_y1_u0( _machflw(_x0_nu(), _y2_u0()));
    }
    // Save delay line offset
...
#pragma Code("dsplib_dsp_fir_init_q31_cfXstY_MAC32x32")
...
#pragma Code("dsplib_dsp_fir_init_q31_cfYstX_MAC32x32")
