...
#pragma Code("dsplib_dsp_dct3_preprocess_init_q15_cfX")

/**
 *  DCT Type III preprocessing initialization function for q15 data type
 *
...
#pragma Code("dsplib_dsp_dct3_preprocess_init_q15_cfY")
...
#pragma Code("dsplib_dsp_dct3_preprocess_q15_XtoY_cfX_MAC16x16")

/**
 *  DCT Type III preprocessing function for q15 data type
 *
 * \param inst - pointer to instance structure
...
void dsp_dct3_preprocess_q15_XtoY_cfX_MAC16x16(const dct3_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output) 
{
    ...
    for (i = 1; i < N; i++) 
    {
        ...
        xSin = _x3_u0();
                       ...
                       _mulrdw(_x0_nu(), xCos);
        _set_y0_u0( _macrdw(_x1_nu(),xSin));
                       _mulrdw(_x1_u0(), xCos);
        _set_y0_u0( _macrdw(_x0_u0(), dspfx_neg_q31(xSin)));
    }
    ...
    _set_y0_u0( dspfx_add_q31(in_temp, _mulrdw(_x0_nu(), _x3_nu())));
    _set_y0_u0( dspfx_sub_q31(in_temp, _mulrdw(_x0_nu(), _x3_nu())));
    ...
    for (i = 1; i < N>>1; i++) {
        ...
        a5 = _macrdw(a3, xSin);
             _mulrdw(a2, xSin);
        a6 = _macrdw(a3, dspfx_neg_q31(xCos));
...
#pragma Code("dsplib_dsp_dct3_preprocess_q15_YtoX_cfY_MAC16x16")
...
void dsp_dct3_preprocess_q15_YtoX_cfY_MAC16x16(const dct3_q15_xy_t *inst, uint32_t offs_input, uint32_t offs_output) 
{
    ...
    for (i = 1; i < N; i++) 
    {
        ...
        xSin = _y3_u0();
                       ...
                       _mulrdw(_y0_nu(), xCos);
        _set_x0_u0( _macrdw(_y1_nu(),xSin));
                       _mulrdw(_y1_u0(), xCos);
        _set_x0_u0( _macrdw(_y0_u0(), dspfx_neg_q31(xSin)));
    }
    ...
    _set_x0_u0( dspfx_add_q31(in_temp, _mulrdw(_y0_nu(), _y3_nu())));
    _set_x0_u0( dspfx_sub_q31(in_temp, _mulrdw(_y0_nu(), _y3_nu())));
...
#pragma Code("dsplib_dsp_dct3_postprocess_q15_XtoY")

/**
 *  DCT Type III postprocessing function for q15 data type
 *
 * \param offs_input[in] - XY memory offset to input vector
 * \param offs_output[out] - XY memory offset to output vector
 * \param length - transform size
...
#pragma Code("dsplib_dsp_dct3_postprocess_q15_YtoX")
