...
#pragma Code("dsplib_dsp_fct4_q15")
...
#define _FCT4_K1(x, y, N2) {                                        \
      ...
      for (p = 0; p < N2; p++) {                                    \
			...
			a = dspfx_sub_q31(_##x##0_u0(), _##x##2_u0());          \
			b = dspfx_asl_q31(b, 1);                                \
            _set_##y##0_u0(dspfx_add_q31(a, b));                    \
            _set_##y##1_u0(dspfx_sub_q31(a, b));                    \
      } /* for p = 0 : N/2-1 */                                     \
                                                                    \
} /* _FCT4_K1 */
    ...
    for (l = 0; l < Delta_nu; l++) {                                \
      for (p = 0; p < N_nu-1; p++) {                                \
        ...
        _set_##y##1_u0(dspfx_sub_q31(a, b));                        \
                   ...
                   a = dspfx_sub_q31(_##x##0_u1(), _##x##2_u1());   \
				   b = dspfx_asl_q31(b, 1);                         \
        _set_##y##0_u1(dspfx_add_q31(a, b));                        \
        _set_##y##1_u1(dspfx_sub_q31(a, b));                        \
    } /* for l = 0 : Delta_nu-1 */                                  \
                                                                    \
} /* _FCT4_K */
...
static _Inline void _fct4_x2y_k1 (int32_t N_nu, int32_t c)
{int32_t p, a, b;    _FCT4_K1 (x, y, N_nu) }
static _Inline void _fct4_y2x_k1 (int32_t N_nu, int32_t c)
{int32_t p, a, b;    _FCT4_K1 (y, x, N_nu) }
static _Inline void _fct4_x2y_k  (int32_t Delta_nu, int32_t N_nu)
{int32_t l, p, a, b; _FCT4_K  (x, y, Delta_nu, N_nu) }
static _Inline void _fct4_y2x_k  (int32_t Delta_nu, int32_t N_nu)
{int32_t l, p, a, b; _FCT4_K  (y, x, Delta_nu, N_nu) }
static _Inline void _fct4_x2y_ke1(int32_t Delta_nu)
{int32_t l, a, b;    _FCT4_KE1(x, y, Delta_nu) }
static _Inline void _fct4_y2x_ke1(int32_t Delta_nu)
{int32_t l, a, b;    _FCT4_KE1(y, x, Delta_nu) }
static _Inline void _fct4_x2y_ke (int32_t Delta_nu)
{int32_t l, a, b;    _FCT4_KE (x, y, Delta_nu) }
static _Inline void _fct4_y2x_ke (int32_t Delta_nu)
{int32_t l, a, b;    _FCT4_KE (y, x, Delta_nu) }
...
/**
 * Initializes instance structure of Fast DCT Type 4 transform
 * for q15 data type. 
 *
 * \param inst - pointer to transform instance structure
 * \param length - transform size
 * \param xyptr_twf - XY memory offset of transform twiddle factors table. Table size: length*2 in X region and length*2 in Y region
 * \result dsp_status
 */

dsp_status dsp_fct4_init_q15_cfXY(fct4_q15_xy_t *inst, int32_t length, int32_t offs_twf) {
  int N2 = length*2;
  if (length > (1 << FCT_MAX_LOGN)){
	return DSP_ERR_LENGTH_NOT_SUPPORTED;
  }
  inst->length = length;
  inst->xyptr_twf = offs_twf;
  memcpy((int16_t*)XMEM + offs_twf, fct_twf_q15, N2*sizeof(int16_t));
  memcpy((int16_t*)YMEM + offs_twf, fct_twf_q15, N2*sizeof(int16_t));
  return DSP_ERR_OK;
} /* fct4_init */
