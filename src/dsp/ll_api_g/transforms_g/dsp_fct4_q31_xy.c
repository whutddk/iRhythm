...
#pragma Code("dsplib_dsp_fct4_q31")

#define HALF   ( 1L<<30)
#define M_HALF (-1L<<30)
#define M_ONE  (-1L<<31)
#define SQRT2  (1518500250L)
// 1 N/2
#define _FCT4_K1(x, y, N2) {                                        \
                                                                    \
      for (p = 0; p < N2; p++) {                                    \
                       _mulflw (_##x##1_nu(), c);                 \
                   b = _machflw  (_##x##1_u0(), c);                 \
                       _machflw(_##x##0_u0(), c2);                \
        _set_##y##0_u0(_machflw  (_##x##2_u0(), c2m) <<1);              \
                       _machflw(           b, c1);                \
        _set_##y##1_u0(_machflw  (           b, c1)<<1);               \
      } /* for p = 0 : N/2-1 */                                     \
                                                                    \
} /* _FCT4_K1 */
...
#define _FCT4_K(x, y, Delta_nu, N_nu) {                             \
    ...
    for (l = 0; l < Delta_nu; l++) {                                \
      for (p = 0; p < N_nu-1; p++) {                                \
                       _mulflw (_##x##1_nu(), _##y##3_nu());      \
                   b = _machflw  (_##x##1_u0(), _##y##3_nu());      \
                       _machflw(_##x##0_u0(), c2);                \
        _set_##y##0_u0(_machflw  (_##x##2_u0(), c2m)<<1);              \
                       _machflw(           b, c1);                \
        _set_##y##1_u0(_machflw  (           b, c1)<<1);               \
                       ...
                       _mulflw (_##x##1_nu(), _##y##3_nu());      \
                   b = _machflw  (_##x##1_u1(), _##y##3_u0());      \
                       _machflw(_##x##0_u1(), c2);                \
        _set_##y##0_u1(_machflw  (_##x##2_u1(), c2m)<<1);              \
                       _machflw(           b, c1);                \
        _set_##y##1_u1(_machflw  (           b, c1)<<1);               \
    } /* for l = 0 : Delta_nu-1 */                                  \
                                                                    \
} /* _FCT4_K */
...
#define _FCT4_LM(x, y, z, N) {                                      \
                                                                    \
    for (l = 0; l < N; l++) {                                       \
                       _mulflw (_##x##0_nu(), _##z##3_nu());      \
        _set_##y##1_u0(_machflw  (_##x##0_u0(), _##z##3_u0()));     \
    } /* for l = 0 : N-1 */                                         \
                                                                    \
} /* _FCT4_LM */
...
#define _FCT4_I_K(x, y, N, Delta_nu, N_nu, in, out, coef) {         \
  ...
  _set_a##y##3(coef + Delta_nu-1);                                  \
                                                                    \
_set_m##x##00(  Delta_nu);/**/                                      \
_set_m##x##10(  Delta_nu);/**/                                      \
  _set_m##x##20((-Delta_nu) & 0x3fff);                              \
  _set_m##y##00(  Delta_nu<<1);                                     \
  _set_m##y##10(  Delta_nu<<1);                                     \
                                                                    \
  _set_m##x##01(( 1 +  Delta_nu     - (N>>1)) & 0x3fff);            \
  _set_m##x##11(( 1 +  Delta_nu     - (N>>1)) & 0x3fff);            \
  _set_m##x##21(  1 -  Delta_nu     + (N>>1));                      \
  _set_m##y##01(( 1 + (Delta_nu<<1) -  N)     & 0x3fff);            \
  _set_m##y##11((-1 + (Delta_nu<<1) -  N)     & 0x3fff);            \
                                                                    \
...
_set_m##x##00(   N>>2);/**/                                     \
_set_m##x##10(   N>>2);/**/                                     \
  _set_m##x##20((-(N>>2))     & 0x3fff);                            \
  _set_m##y##00(   N>>1);                                           \
  _set_m##y##10(   N>>1);                                           \
                                                                    \
  _set_m##x##01(( 1 - (N>>2)) & 0x3fff);                            \
  _set_m##x##11(( 1 - (N>>2)) & 0x3fff);                            \
  _set_m##x##21(  1 + (N>>2));                                      \
  _set_m##y##01(( 1 - (N>>1)) & 0x3fff);                            \
  _set_m##y##11((-1 - (N>>1)) & 0x3fff);                            \
                                                                    \
...
dsp_status dsp_fct4_init_q31_cfXY(fct4_q31_xy_t *inst, int32_t length, int32_t offs_twf) {
  int N2 = length*2;
  ...
  memcpy((int32_t*)XMEM + offs_twf, fct_twf_q31, N2*sizeof(int32_t));
  memcpy((int32_t*)YMEM + offs_twf, fct_twf_q31, N2*sizeof(int32_t));
  return DSP_ERR_OK;
} /* fct4_init */

/**
 * Fast DCT Type 4 transform "process" function for q31 data type
 *
