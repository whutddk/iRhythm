...
#pragma Code("dsplib_dsp_cmpl_mag_q31_XtoY_MAC32x32")

/**
 * Computes the Q31 magnitude of complex numbers stored in a vector                      
 * The return result is in 1.30 format.
 *
...
void dsp_cmpl_mag_q31_XtoY_MAC32x32(uint32_t offs_Src, uint32_t offs_Dst, uint32_t nSamples)
{
    ...
    {
        q31_t temp, root;
                _mullw(_x0_nu(), _x0_nu());
               _machlw(_x0_u0(), _x0_nu());                
                _maclw(_x0_nu(), _x0_nu());
        temp = _machlw(_x0_u0(), _x0_nu());
...
#pragma Code("dsplib_dsp_cmpl_mag_q31_YtoX_MAC32x32")
...
void dsp_cmpl_mag_q31_YtoX_MAC32x32(uint32_t offs_Src, uint32_t offs_Dst, uint32_t nSamples)
{
    ...
    {
        q31_t temp, root;
                _mullw(_y0_nu(), _y0_nu());
               _machlw(_y0_u0(), _y0_nu());                
                _maclw(_y0_nu(), _y0_nu());
        temp = _machlw(_y0_u0(), _y0_nu());
        root = dsp_sqrt_q31(temp>>1);
        _set_x0_u0(root);
    }
};
