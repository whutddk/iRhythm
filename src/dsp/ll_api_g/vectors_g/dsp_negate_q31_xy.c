...
#pragma Code("dsplib_dsp_negate_q31_XtoY")

/**
 * Q31 vector negation value. Computes the 
 * negate value of a vector on an element-by-element basis.
 *
...
void dsp_negate_q31_XtoY(int32_t offs_Src, int32_t offs_Dst, uint32_t nSamples) 
{
    ...
    for (sample = 0; sample < (int32_t)nSamples>>1; sample++){
        _set_y0_u0( dspfx_neg_q31(_x0_u0()));
        _set_y0_u0( dspfx_neg_q31(_x0_u0()));
    }
    if (nSamples & 1)
        _set_y0_u0( dspfx_neg_q31(_x0_u0()));
...
#pragma Code("dsplib_dsp_negate_q31_YtoX")
...
void dsp_negate_q31_YtoX(int32_t offs_Src, int32_t offs_Dst, uint32_t nSamples) 
{
    ...
    for (sample = 0; sample < (int32_t)nSamples>>1; sample++){
        _set_x0_u0( dspfx_neg_q31(_y0_u0()));
        _set_x0_u0( dspfx_neg_q31(_y0_u0()));
    }
    if (nSamples & 1)
        _set_x0_u0( dspfx_neg_q31(_y0_u0()));
