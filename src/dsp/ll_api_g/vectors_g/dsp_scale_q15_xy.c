...
#pragma Code("dsplib_dsp_scale_q15_XtoY_MAC16x16")
...
{
    int32_t sample;
    q31_t scale_q31 = dspfx_asl_q31(scaleFract, 16);
    ...
    {
        _muldw(_x0_u0(), scale_q31);
        _set_y0_u0( dspfx_asl_q31(_get_ACC1(), shiftBits));
    }
}
// Produce instance with: MAC16x16; pIn0 - Y; 

#pragma Code("dsplib_dsp_scale_q15_YtoX_MAC16x16")
...
void dsp_scale_q15_YtoX_MAC16x16(int32_t offs_Src, q15_t scaleFract, int8_t shiftBits, int32_t offs_Dst, uint32_t nSamples) 
{
    ...
    for (sample = 0; sample < (int32_t)nSamples; sample++)
    {
        _muldw(_y0_u0(), scale_q31);
        _set_x0_u0( dspfx_asl_q31(_get_ACC1(), shiftBits));
    }
}
